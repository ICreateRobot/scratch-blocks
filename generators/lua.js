 ...
 */
Blockly.Lua.variables_ = Object.create(null);
Blockly.Lua.INDENT='  '
/**
...
Blockly.Lua.init = function(workspace) {
  ...
  if (!Blockly.Lua.variableDB_) {
    Blockly.Lua.variableDB_ = new Blockly.Names(Blockly.Lua.RESERVED_WORDS_);
  } else {
  ...
  for (var i = 0; i < variables.length; i++) {
    const varName = Blockly.Lua.variableDB_.getName(variables[i].name, Blockly.Variables.NAME_TYPE);
    Blockly.Lua.variables_[i] = varName + " = 0";  // 可以根据变量类型初始化为其他值
  }
...
Blockly.Lua.finish = function(code) {
  ...
  // // 1. 提取 definitions 和 variables
  // const definitions = Object.values(Blockly.Lua.definitions_ || {});
  // const variables = Object.values(Blockly.Lua.variables_ || {}); // 新增变量部分
  ...
  // this.nameDB_.reset();
  // delete Blockly.Lua.definitions_;
  // delete Blockly.Lua.variables_; // 清除变量缓存

  // // 4. 拼接结构：definitions + variables + code
  // return definitions.join('\n\n') + '\n\n' +
  //        variables.join('\n') + '\n\n' +  // 新增变量输出
  //        code;
};
